...
float current_noise_baseline = 0.0;  // RMS noise with no load
...
#define CURR_PIN 34
int curr_offset = 2900;          // ✓ FIXED: removed 'const' so calibration can update it
...
// ✓ FIXED: Raised thresholds to eliminate phantom current
const float NOISE_FLOOR = 0.20;            // ✓ Raised from 0.10
const float CONFIRMATION_THRESHOLD = 0.25; // ✓ Raised from 0.15
const float DIRECT_ACCEPT = 0.40;          // ✓ Raised from 0.30
...
// Hour-wise consumption tracking
...
void setup() {
  ...
  Serial.println("║  ✓ FIXED Phantom Current Readings     ║");
  ...
  Serial.println("\n✓ System ready!");
  Serial.println("✓ Voltage threshold: 150V");
  Serial.println("✓ Current noise floor: 0.20A");
...
void calibrateOffsets() {
  ...
  // ✓ NEW: Measure noise baseline (RMS with no load)
  // This captures the sensor's inherent noise floor
  long sumNoise = 0;
  for (int i = 0; i < 2000; i++) {
  ...
  current_noise_baseline = (noiseRMS / curr_sensitivity) * 1.5;  // 1.5x safety margin

  Serial.print("  V-offset: ");
  Serial.print(volt_offset);
  Serial.print(" | I-offset: ");
  Serial.print(curr_offset);
  Serial.print(" | Noise baseline: ");
  Serial.print(current_noise_baseline, 4);
  Serial.println("A");
}
...
float measureCurrentRaw() {
  ...
  // ✓ NEW: Subtract noise baseline
  current = current - current_noise_baseline;
...
float smartFilter(float rawCurrent) {
  ...
  if (smoothCurrent > 0.25 && consecutiveHighReadings > 5) {
    return rawCurrent;
  }
  ...
  if (currentSupplyState) {
    ...
    if (filteredCurrent > 0) {
      ...
      if (consecutiveLowReadings > 7) {
        ...
        if (smoothCurrent < 0.05) smoothCurrent = 0.0;
      } else {
