...
// ============================================================
// MONTHLY INDIVIDUAL RECORD UPLOAD
// Uploads each month separately to /monthly_records/USER001/
// Structure per record:
//   bill_period_from  → "2024-01-01"
//   bill_period_to    → "2024-01-31"
//   month_units       → 140.00
//   total_bill        → 1650.00   (calculated on that month's units alone)
//   due_date          → "2024-03-15"  (from the bi-monthly bill due date)
...
void uploadMonthlyRecord(int monthIdx, int year, float monthUnits, String dueDateStr) {
  ...
  // Bill calculated on this month's units only (for individual record)
  ...
  if (httpCode == 200 || httpCode == 201) {
    Serial.print("✓ Monthly record uploaded | ");
    Serial.print(String(monthNames[monthIdx])); Serial.print(" ");
    Serial.print(year); Serial.print(" | Units: ");
    Serial.print(monthUnits); Serial.print(" | Bill: ₹");
    Serial.println(monthBill);
  } else {
...
void sendBiMonthlyBillToAdmin() {
  ...
  // ✓ Upload individual monthly records for BOTH months
  uploadMonthlyRecord(m1Idx, cycleYear, m1Units, String(dueDateStr));
  uploadMonthlyRecord(m2Idx, cycleYear, m2Units, String(dueDateStr));
